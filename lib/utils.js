...
function ensureWindows () {
  ...
  const major = parseInt(release.slice(0, 2), 10)
  const minor = parseInt(release.slice(3, 4), 10)
  const build = parseInt(release.slice(5), 10)
...
function isDirectory (directory) {
  return pathExistsSync(directory)
}
...
function debug (message) {
  _debug = _debug || debugLib('electron2appx')
  _debug(message)
}
